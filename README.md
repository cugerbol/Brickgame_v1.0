...
Демонстрация моей реализации проекта школы 21 - BrickGame v1.0, или же просто Tetris. Проект сделан в функциональном стиле на языке С. Цель данного проекта отработать навык и углубить знания в следующих темах:
...
Ниже приведены задания и требования к проекту, а так же изображения со схемой конечного автомата и кадрами из игры.

![fsm](images/fsm.png)

![game](images/game.png)

Проект собирается в терминале через Makefile и все команды работают из корневой папки проекта. Чтобы запустить какую либо из целей не обходимо написать `make` и название этой цели
Цели в Makefile:

- `make install` - устанавливает игру и создает бинарный файл tetris.
- `make uninstall` - удаляет все созданные объектные файли и файл игры.
- `make test` - собирает проект если он не собран и тестирует все функции на корректность их работы.
- `make check` - устанавливает игру если она не установления и сразу запускает ее. Процесс работы программы анализируется на учетки памяти и по заврешению игры, выводит в терминал результат проверки.
- `make gcov_report` - запускает тестирование функций и создает html отчет о покрытии функций при тестировании, по завершению открывает отчет в браузере
